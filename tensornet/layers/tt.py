...
def binarized_tt_v2(inp,
    ...
    with tf.variable_scope(scope):
        ...
        inp = tf.Print(inp, [tf.shape(inp)], message="inp=")
        Wr =  tf.ones_like(out)
        # Wr = tf.Print(Wr, [tf.shape(Wr)], message="Wr=")
        ...
        for i in range(dim):
            ...
            Wr = tf.Print(Wr, [tf.shape(Wr)], message="Wr=")
            core = tf.Print(mat_cores[i], [tf.shape(mat_cores[i])], message="Mat_core_{}=".format(i))
            Wr = tf.matmul(core, Wr)
            Wr = tf.reshape(Wr, [out_modes[i], -1])
            Wr = tf.transpose(Wr, [1, 0])   
        tf.clip_by_value(Wr, -1, 1)     
        Wb = binarize(Wr)
        Wb = tf.Print(Wb, [tf.shape(Wb)], message="Wb=")
        if binarize_input:
            out = binarize(out)
        out = tf.reshape(out, [out_modes[-1], -1])
        # out = tf.transpose(out, [1, 0])
        out = tf.Print(out, [tf.shape(out)], message="out=")
        out = tf.matmul(Wb, out)
        if biases_initializer is not None:
...
def bnn_layer(inp,
    ...
    with tf.variable_scope(scope):
        ...
        # print("SHAAAAAAAAAAAAAPPPPPPPPPPPPE", out.get_shape())
