...
opts = {}
opts['inp_modes_1'] = np.array([2, 2, 2, 2, 7, 7], dtype='int32')
...
def inference(images, train_phase):
    ...
    # images = tf.Print(images, [images], "images = ")
    layers = []
    layers.append(images)

    # layers[-1] = tf.Print(layers[-1], [layers[-1]], "layers = ")
...
def loss(logits, labels):
    ...
    # onehot_labels = tf.Print(onehot_labels, [onehot_labels[0]], message="One Hot Labels =")
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits,
                                                            labels=tf.cast(onehot_labels, tf.float32),
                                                            name='xentropy')
