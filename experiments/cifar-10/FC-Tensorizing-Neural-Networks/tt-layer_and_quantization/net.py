...
inp = opts[0]
out = opts[1]
r1_full = 3#min(inp[0]*out[0], np.prod(inp[1:]* out[1:]))
r2_full = 3#min(np.prod(inp[:1]*out[:1]), np.prod(inp[2:]* out[2:]))
r3_full = 3#min(np.prod(inp[:2]*out[:2]), np.prod(inp[3:]* out[3:]))
r4_full = 3#min(np.prod(inp[:3]*out[:3]), np.prod(inp[4:]* out[4:]))
r5_full = 3#min(np.prod(inp[:4]*out[:4]), inp[5], out[5])
...
# opts[11] = np.array([1, r1_full, r2_full, r3_full, r4_full, r5_full, 1], dtype='int32')

opts['use_dropout'] = True
opts['learning_rate_init'] = 0.01
opts['learning_rate_decay_steps'] = 2000
opts['learning_rate_decay_weight'] = (0.0003/opts['learning_rate_init'] )**(1./opts['learning_rate_decay_steps']) # From BNN
